...
import './ResultReactTable.css';
import { getServerData } from '../helper/helper';
import axios from 'axios';
import Swal from 'sweetalert2';

const ResultReactTable = ({ totalAttempts }) => {
  const [data, setData] = useState([]);
  const [selectedUser, setSelectedUser] = useState(null);
  const [email, setEmail] = useState('');

  useEffect(() => {
    getServerData('http://localhost:5000/api/reactquiz/resultReact', (res) => {
      setData(res);
    });
  }, []);
  ...
  const handleSendEmail = () => {
    if (email && selectedUser) {
      const htmlContent = `
        ...
        <p>You have successfully passed the ReactJS quiz and earned your certificate from Ras E-learning platform.</p>
        <table>
          <thead>
            <tr>
              <th>Attempts</th>
